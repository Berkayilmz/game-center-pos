// src/modules/pos/cari/components/PaymentModal.jsx
...
const PaymentModal = ({ open, onClose, onSave, editItem }) => {
  const [activeTab, setActiveTab] = useState("info");
  if (!open) return null;

  const initialValues = editItem || {
    date: new Date().toISOString().slice(0, 10),
    docNo: `PAY-${Date.now().toString().slice(-4)}`,
    dueDate: "",
    amount: "",
    description: "",
    account: "",
    ...
    <div className="modal-overlay">
      <div className="modal-box large">
        ...
        <Formik
          ...
          onSubmit={(values) => {
            const balance = (values.credit || 0) - (values.debt || 0);
              ...
              {activeTab === "notes" && (
                <div className="form-col full-width">
                  ...
                  <Field
                    ...
                    rows="6"
                    className="form-input"
                    placeholder="Bu ödeme ile ilgili notlar..."
                  />
                </div>
              )}
