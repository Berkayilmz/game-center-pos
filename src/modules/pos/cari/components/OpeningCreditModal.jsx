// src/modules/pos/cari/components/OpeningCreditModal.jsx
import React, { useState } from "react";
import { Formik, Form, Field, ErrorMessage } from "formik";
import * as Yup from "yup";
import "../../inventory/inventory.css";

const OpeningCreditSchema = Yup.object().shape({
...
const OpeningCreditModal = ({ open, onClose, onSave, editItem }) => {
  const [activeTab, setActiveTab] = useState("info");
  if (!open) return null;

  const initialValues = editItem || {
    date: new Date().toISOString().slice(0, 10),
    docNo: `AÇA-${Date.now().toString().slice(-4)}`,
    dueDate: "",
    amount: "",
    description: "",
    account: "",
    ...
    <div className="modal-overlay">
      <div className="modal-box large">
        ...
        <Formik
          ...
          validationSchema={OpeningCreditSchema}
          onSubmit={(values) => {
              ...
              {activeTab === "notes" && (
                <div className="form-col full-width">
                  ...
                  <Field
                    ...
                    rows="6"
                    className="form-input"
                    placeholder="Bu açılış alacağı ile ilgili notlar..."
                  />
                </div>
              )}
...
export default OpeningCreditModal;