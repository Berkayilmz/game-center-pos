// src/modules/pos/cari/components/TransferDebtModal.jsx
import React, { useState } from "react";
import { Formik, Form, Field, ErrorMessage } from "formik";
import * as Yup from "yup";
import "../../inventory/inventory.css";

const TransferDebtSchema = Yup.object().shape({
...
const TransferDebtModal = ({ open, onClose, onSave, editItem }) => {
  const [activeTab, setActiveTab] = useState("info");
  if (!open) return null;

  const initialValues = editItem || {
    date: new Date().toISOString().slice(0, 10),
    docNo: `DEVB-${Date.now().toString().slice(-4)}`,
    dueDate: "",
    amount: "",
    description: "",
    account: "",
    ...
    <div className="modal-overlay">
      <div className="modal-box large">
        ...
        <Formik
          ...
          validationSchema={TransferDebtSchema}
          onSubmit={(values) => {
            const balance = (values.credit || 0) - (values.debt || values.amount);
            const final = {
              ...values,
              debt: values.amount,
              balance,
              ...
              {activeTab === "notes" && (
                <div className="form-col full-width">
                  ...
                  <Field
                    ...
                    rows="6"
                    className="form-input"
                    placeholder="Bu devir borcu ile ilgili notlar..."
                  />
                </div>
              )}
...
export default TransferDebtModal;